...
const PerformanceMap = () => {
    const defaultMarkers = [
      ...
      const labels = [
        { color: "#D5F15B", text: "Чандагатай", percent: 50 },
        { color: "#3478F6", text: "Төвийн урд", percent: 50 },
        { color: "#203E7D", text: "Төвийн урд", percent: 50 },
      ];
...
//   const ulaanbaatarCoords = [47.918873, 106.917701]; // Default center coordinates
  ...
  const handleMapClick = (latlng) => {
    console.log("Marker added at: ", [...markers, latlng]);
      ...
      <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
        ...
        {markers.map((position, idx) => (
          <Marker key={idx} position={position}>
            <Popup>
              Tag #{idx + 1} <br /> Lat: {position.lat.toFixed(4)}, Lng: {position.lng.toFixed(4)}
            </Popup>
          </Marker>
        ))}
      </MapContainer>

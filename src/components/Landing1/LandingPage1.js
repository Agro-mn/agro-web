import {Col, Row, Affix, Flex, Menu, Button, Modal, Anchor, ConfigProvider} from 'antd';
      ...
      >
          <Row>
              <Col span={24} style={{position: "absolute", zIndex: 1, width: '100vw'}}>
                  <Affix>
                      <Row style={{height: '5rem', padding: '0 2rem', margin: "2rem", color: 'white', backgroundColor: "rgba(255,255,255,0.34)", backdropFilter: "blur(10px)", boxShadow: "0 1px 9px solid #ffffff", borderRadius: '20px'}}>
                              ...
                              {!loggedUser &&
                                  <Flex align='center' justify='center' style={{ height: '100%', }}>
                                      <Anchor direction='horizontal'   >
                                          {items.map((item) => {
                                              return <Anchor.Link title={<a style={{color: `${themeData.colorPrimary}`, fontSize: '18px', margin: '0px 10px'}}>{item.label}</a>} key={item.key} href={`#${item.key}`} />
                                          })}
                                      </Anchor>
                                  </Flex>
                              }
                          ...
                          <Col xs={24} sm={12} md={12} lg={6} xl={6} xxl={6}>
                              <Flex align='center' justify='right' gap='middle' style={{ height: '100%', marginRight: '1rem' }}>
                                  ...
                                  <Button type='text' style={{color: `${themeData.colorPrimary}`, padding: '1.2rem 1.5rem', border: `3px solid ${themeData.colorPrimary}`, borderRadius: '10px'}}>Бүртгүүлэх</Button>
                                  <Button style={{background: `${themeData.colorPrimary}`, padding: '1.2rem 1.5rem',color: '#ffffff', border: `3px solid ${themeData.colorPrimary}`, borderRadius: '10px'}} onClick={() => { showModal(); }}>{loggedUser ?? 'Нэвтрэх'}</Button>
                              </Flex>
                          </Col>
                          <Modal
                              width={'30%'}
